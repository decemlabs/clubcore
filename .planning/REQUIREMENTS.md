...
- [ ] **FND-01**: `apps/admin-app` is part of the clubcore repo and builds/runs/tests there; the package-manager/workspace decision (Bun → pnpm vs standalone) is made at the Phase 100 plan; a dedicated CI job runs its `check` + `test` + `build`.
...
- [ ] **AUTH-01**: a staff member logs in via the real `/api/v1/auth` (email/password); the session persists across a browser refresh and the CSRF token is captured for mutations.
- [ ] **AUTH-02**: unauthenticated navigation redirects to `/login`; logout ends the session; Settings lists active sessions (`/auth/sessions`) and can revoke them.
- [ ] **AUTH-03**: the UI reflects the staff role (owner vs reception) — owner-only screens/actions are hidden or disabled for reception, and a backend `403` surfaces as a friendly state, never a crash.

### CLI — Clients

- [ ] **CLI-01**: the Clients list renders real `GET /api/v1/clients` with server-side search + the pagination envelope `{items,total,page,pageSize}`, including loading / error / empty states.
- [ ] **CLI-02**: the Client detail screen renders real `GET /api/v1/clients/{id}` plus that client's memberships / visits / payments.
- [ ] **CLI-03**: create + edit client persist via `POST` / `PATCH /api/v1/clients` (Zod-validated form, money in kopecks); soft-delete via `DELETE`.

### MEM — Memberships & Plans

- [ ] **MEM-01**: the Plans/Абонементы screen lists real membership plans (`/membership-plans`) and PT-package plans (`/pt-package-plans`); create/edit are owner-only gated.
...
- [ ] **SCH-01**: the Schedule screen renders real trainer slots (`/trainer-slots`), recurring templates (`/recurring-templates`) and time-off (`/time-off`); create/edit are owner-only.
...
- [ ] **TRN-01**: the Trainers list + Trainer detail render real `/trainers` (catalog + bio/specialization).
...
- [ ] **SET-01**: Settings (profile) reads/edits the current staff profile + theme, and manages active sessions.
