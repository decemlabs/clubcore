...
- [ ] **INFRA-23**: Lock canonical payload schemas for the 16 new audit events in `audit_payloads.py` (or equivalent). `payment_refunded` payload MUST include `payment_row_hash` (SHA-256 canonical-JSON of original payment row) for forensic chain-of-custody.
