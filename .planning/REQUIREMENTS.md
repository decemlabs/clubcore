...
- [ ] **PAY-01**: Owner настраивает на тренере модель компенсации — `commission_pct` (NUMERIC) и/или `session_fee_kopecks` (INT), оба nullable; оба NULL = payroll для тренера не считается; гибрид (оба заданы) допускается
