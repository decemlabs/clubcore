...
| bug→v2.6 | **Typing indicator does NOT render** in the PWA: WS typing frame delivered + `window.__chatTyping` bridge fires, but `setTyping(true)` doesn't surface the dots (found in browser verify 2026-06-08). Dormant in v2.5 (no typing producer until v2.6). Needs `/gsd:debug` + React DevTools. See 94-HUMAN-UAT.md Gaps. | deferred → v2.6 (fix with the typing producer) |
