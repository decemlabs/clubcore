...
- 25 and 26 share migration `0007` — sequential is safer than parallel; alembic migration must be a single combined revision authored in 25, then extended in 26 OR a shared "0007_freeze_and_renewal" prepared in 25 covering both columns. Plan-time decision.
- 27 depends on 25 (frozen filter in cron select) and on 24 (`expiring_notification_sent_*` in `LOCKED_AUDIT_EVENTS`); does **not** depend on 26 logically, but migration `0008` must come after `0007` regardless.
...
- **Migration `0007` совмещает freeze + renewal columns** — план-агент Phase 25 должен либо подготовить combined revision, либо дать Phase 26 чистый extension hook.
