...
| v2.1 | no new domains; additive fields on existing schemas (membership priceKopecks/autoRenew, clients.notif_prefs JSONB — migrations 0050/0051 incl. FIT15 seed); +2051/−169 LOC across 29 files, mostly `apps/client-pwa` wiring | `apps/admin-web` unchanged (frozen); `apps/client-pwa` extended — newbie-Home/Profile/Settings/Checkout wired to the v2.0 client API; localStorage notif store dropped for server-backed | 10/10 (audit `tech_debt`, 0 blockers; live human-verify checks + pre-existing debt deferred) | backend ruff/mypy-strict/import-linter green on touched files; PWA lint/tsc/build + 92/92 Vitest green; code review caught 2 camelCase BLOCKERS (P76) + cancel real-id BLOCKER (P77) + 3 NOTIF races (P78), all fixed inline |
| v2.2 | new `app/modules/payment_methods/` + read-only `/client/activity/weekly`; Alembic 0052 (client_payment_methods) + 0053 (booking_notifications kind widen); staff paths byte-identical, client-portal paths additive | `apps/admin-web` unchanged (frozen); `apps/client-pwa` — CardSheet + activity bars + checkout save-card opt-in wired to real endpoints; `linkedCard`/`weeklyActivity` flipped ON; mock calendar + per-booking autopay toggle removed | 11/11 (audit `passed`; PAYM-01 PWA-capture gap closed inline by Phase 81.1; live-infra checks OPERATOR-PENDING) | backend mypy-strict/lint-imports green; 185 client_portal+bookings integration tests; PWA 130/130 Vitest; byte-stable openapi.json + schema.d.ts drift gates; code review caught 4 real bugs across phases (consent-wipe, post-commit-DM-500, idempotency-envelope-corruption, fake-TZ-test), all fixed inline |

## Milestone: v2.2 — Membership self-service depth

**Shipped:** 2026-06-03 | **Phases:** 4 (79-81 + closure 81.1) | **Plans:** 11

### What Was Built
Card-on-file (migration 0052, `payment_methods/` module, webhook step-8.5 token capture, GET/DELETE/PATCH endpoints, UI-only autopay with ФЗ-376 consent), booking reschedule (atomic cancel+create, PT-credit preserved, audit + DM), and weekly-activity analytics (`/client/activity/weekly` over the `visits.gym_date` STORED column). PWA flag-flips wired CardSheet + activity bars; Phase 81.1 closed the checkout save-card capture gap.

### What Worked
- **Code review caught 4 real bugs the tests missed** — and in two cases (P80 fire-and-forget DM, P81 fake-TZ-test) the bug was *introduced by following an earlier plan-checker note literally*. The review→fix loop is doing real work, not ceremony.
- **The milestone audit's cross-phase integration check earned its keep** — it found the PAYM-01 capture gap (backend + management UI shipped, but no surface sent the flag) that every per-phase verification missed because each phase was individually complete.
- **Sequential-on-main execution** (over parallel worktrees) avoided shared-Postgres test races for backend phases — correctness over the marginal parallelism of one 2-plan wave.

### What Was Inefficient
- The save-card capture gap should have been caught at planning: Phase 79 was backend-only and Phase 81's PAYM-05 covered card *management*, but no phase owned card *capture* at checkout. A milestone-level "who wires the user-facing entry point?" check during roadmap would have pre-empted the 81.1 closure.

### Key Lessons
1. **Per-phase verification ≠ milestone integration.** A feature can be 3/3 phases green yet have no end-to-end entry point. The cross-phase audit is the only gate that catches this — keep running it before complete.
2. **Plan-checker notes can induce bugs.** "Make sure the DM is sent" → executor moved a network send into the post-commit path and could 500 a durable write. Tighten such notes with the *non-functional* constraint (fire-and-forget, don't hold the txn) not just the functional one.
3. **A "golden test" that re-implements the formula tests nothing.** The TZ test must exercise the real STORED column end-to-end, not Python `astimezone` arithmetic.
