...
**🚧 v1.9 Trainers Complete — in progress.** Phase 58 (Payroll Foundations + Ledger) ✅ complete 2026-05-25 (9 plans; PAY-01..06): new `app/modules/payroll/` ships owner-only versioned comp-config (INSERT-only), read-only accrual preview, append-only `trainer_payroll_accruals` ledger (ON CONFLICT idempotency + run-time rate snapshot + single pending→paid transition), paginated list, and a `PayrollClawbackRecorder` Protocol slot wiring a same-UoW negative clawback into the PT-package refund flow (zero new `ignore_imports`). Integer `math.ceil` commission rounding in the trainer's favor (D-58-04, ratified over the earlier banker's-rounding intent). 4 new LOCKED_AUDIT_EVENTS + 5 OWNER_ONLY pairs, three-way RBAC parity green. Remaining v1.9: Phase 59 (recurring schedule + time-off), 60 (trainer-usage report), 61 (OpenAPI handoff + milestone verification). This brings Trainers from catalog-only toward ✅ — the last incomplete business domain.
