...
**v1.4 Cash Sales + PT Packages — shipped 2026-05-16** (7 phases, 22 plans, 61/61 v1.4 in-scope requirements satisfied; ~12K LOC backend Python; admin-web frozen at v1.3 baseline per the 2026-05-15 pivot; 1027 backend pytest passing + 270 admin-web vitest passing). Backend now ships the full cash-sales business core: append-only `payments` ledger with signed kopecks + CHECK-enforced sign/subject correlation, owner-only trainers catalog with soft-delete, PT-package plans + race-safe instances with auto-expire ARQ cron, race-safe PT-session recording with atomic decrement + trainer-name snapshot, refund flow with frozen + renewed-source guards, full audit chain with 51 locked events + `payment_row_hash` SHA-256 traceability + Pydantic-enforced payload schemas. OpenAPI artifact (`openapi.json` + `schema.d.ts`) regenerated byte-stably with 36 compile-time `AssertNonNever` forward-guards as the formal v1.5 design-team handoff contract; v1.4 handoff drafts (auto-generated Postman v2.1 collection + auth setup runbook) committed to `.planning/handoff/` to unblock independent frontend work. Phase 36 served as the backend-only milestone-verification gate (adapted from v1.3 Phase 29 discipline) — 8/8 operator API-contract scenarios via curl against live `docker compose up` stack + 20/20 race tests across 7 files + 4/4 backend CI gates green + admin-web informational canary green; 5 production-blocker regressions discovered and fixed inline at the gate (REG-36-01 arq-worker compose `uv run` → `arq`; REG-36-02 redis port for host pytest; REG-36-03 pt_sessions UUID stringify in audit JSON; REG-36-04 race-test identity-map `expire_all()`; REG-36-05 ruff cleanup of 21 pre-existing errors) at the D-36-17 5-blocker hard cap. 44 pre-existing pytest failures (A/B-verified against Phase 35 close commit `53335c7`) rolled forward as DEFER-36-04-A — recommended single-commit fix is `pt_packages/service.py` UUID stringify, which unlocks ~28 of 44 (same defect class as REG-36-03). FE-11..18 (8 reqs — admin-web wiring of payment/trainer/PT flows) descoped to v2.0 Frontend Integration milestone per the 2026-05-15 pivot.
...
**v1.5 API Handoff for Design Team** — formalize what Phase 36 shipped as drafts. Build out the consumer-facing surface that lets the external design team develop both production frontends (admin + client) independently of this repo: curated Postman v2.1 collection (environments + pre-request login scripts + per-endpoint sample bodies) on top of the v1.4 draft; Newman CLI runner that turns the collection into an executable contract-test suite (run on push); auth setup runbook expanded with sample integration flows + error-envelope cookbook + Telegram OTP deep-link recipe; OpenAPI doc site (hosted spec viewer — likely Redoc / Stoplight Elements / Scalar) + versioned URL for the spec (GitHub Pages or S3 mirror) so the design team can pull a canonical artifact; `@sportzal/api-client` published from `0.0.0` → `0.1.0` to npm (or a private registry); OpenAPI tag curation pass + explicit `operation_id=` decorators on routes (now that the design team can give feedback on generated method names); idempotency hardening per Phase 33 carry-over (CR-01 — bind route into `_redis_key`; CR-02/02b — adopt `idempotent_response` helper across all PT-package mutating endpoints); pt_packages UUID-in-audit-JSON stringify fix that unlocks DEFER-36-04-A (~28 of 44 pre-existing pytest failures) as the v1.4.1 / Phase 36.1 hot-fix opener; ruff format normalization pass per DEFER-36-04-B. No new business-domain features in v1.5 — all of it is contract polish + tech-debt clearance to make the v2.0 Frontend Integration milestone executable.

| Milestone | Focus |
|---|---|
| **v1.5 — API Handoff for Design Team (next)** | Curated Postman collection + Newman runner; auth runbook expansion; OpenAPI doc site + versioned spec URL; `@sportzal/api-client` publish; tag/operationId curation; idempotency CR-01/02/02b hardening; DEFER-36-04-A/B clearance |
| **v1.6 — Reports + Audit Log** | Backend API for owner dashboard: revenue by day/month, active/expiring clients, `GET /api/v1/audit-log`. UI consumed by external admin via OpenAPI |
| **v1.7 — Online Payments (ЮKassa)** | ЮKassa intake + webhooks + 54-ФЗ fiscal receipts |
| **v2.0 — Frontend Integration** | Design team ships production admin + client apps; integration tests at the seam (live backend + real frontends); migration or coexistence with `apps/admin-web` (mock reference) |
