...
**v1.4 Cash Sales + PT Packages — shipped 2026-05-16** (7 phases, 22 plans, 61/61 v1.4 in-scope requirements satisfied; ~12K LOC backend Python; admin-web frozen at v1.3 baseline per the 2026-05-15 pivot; 1027 backend pytest passing + 270 admin-web vitest passing). Backend ships the full cash-sales business core: append-only `payments` ledger with signed kopecks + CHECK-enforced sign/subject correlation, owner-only trainers catalog with soft-delete, PT-package plans + race-safe instances with auto-expire ARQ cron, race-safe PT-session recording with atomic decrement + trainer-name snapshot, refund flow with frozen + renewed-source guards, full audit chain with 51 locked events + `payment_row_hash` SHA-256 traceability + Pydantic-enforced payload schemas. OpenAPI artifact (`openapi.json` + `schema.d.ts`) regenerated byte-stably with 36 compile-time `AssertNonNever` forward-guards; v1.4 handoff drafts (auto-generated Postman v2.1 collection + auth setup runbook) committed to `.planning/handoff/` as **drafts to be polished in the v1.9 API Handoff milestone**.

Phase 36 served as the backend-only milestone-verification gate — 8/8 operator API-contract scenarios via curl against live `docker compose up` stack + 20/20 race tests + 4/4 backend CI gates green; 5 production-blocker regressions discovered and fixed inline at the D-36-17 5-blocker hard cap. **Phase 36.1 pre-v1.5 hot-fix (commit `118eb70`, 2026-05-16) cleared 33 of 44 DEFER-36-04-A pytest failures via 4 mechanical fixes** (pt_packages UUID stringify ×3, alembic env import, RBAC count assert refresh, test_revert_predicate column cleanup); clean-DB pytest now 1060 passing / 11 remaining failures rolled into v1.5+ cycle (7 pt_sessions MissingGreenlet + 3 pt_packages validation_error envelope drift + 1 test_revert_predicate partial-UNIQUE logic bug).

Cumulative shipped versions: v1.0 (Skeleton, 47/47), v1.1 (Auth + Clients, 70/70), v1.2 (Memberships + Visits, 63/63), v1.3 (Memberships Extras + Tech-Debt, 44/44), v1.4 (Cash Sales + PT Packages, 61/61). See `MILESTONES.md` for full history and `.planning/milestones/v1.X-ROADMAP.md` for per-milestone phase breakdowns.
...
| Schedule + Bookings | ❌ placeholder modules | **v1.5 fills this gap (PT slots only)** |

Reports + Audit Log read API: write-side complete (51 LOCKED events), read-side scheduled for v1.8.

## Next Milestone Goals

**v1.5 Schedule + Bookings (PT slots)** — close the last untouched business-module gap. The empty placeholders `app/modules/schedule/` and `app/modules/bookings/` get a minimal viable PT-booking model: a trainer publishes 1:1 availability windows, a client books an individual PT session online (via reception, owner, or Telegram bot `/book`), the booking enters a `confirmed → cancelled / no_show / completed` FSM with race-safe partial-UNIQUE guard, and `completed` is naturally triggered when a PT-session is recorded against that booking. **Group classes are explicitly out of scope** — Sportzal operates an open-gym model for non-PT activity; if group classes become a real requirement they will be a separate later milestone, not a v1.5 sub-scope. Booking still requires an active PT-package with `sessions_remaining > 0`; the atomic decrement remains on **PT-session creation** (not on booking) — this preserves the v1.4 invariant that sessions are debited at delivery time. 5 new audit events (`slot_published`, `booking_created`, `booking_cancelled`, `booking_no_show`, `booking_completed`) bring `LOCKED_AUDIT_EVENTS` from 51 → 56. Phase numbering continues from v1.4 (starts at Phase 37).

The full path to production launch was re-ordered on 2026-05-17 under the explicit strategy **"finish all business modules first → polish → integrate → launch"**. The previous v1.5 "API Handoff" plan is intentionally pushed to v1.9 so the contract is frozen *after* every business surface has landed — otherwise the handoff artifacts would go stale within two milestones.

| Milestone | Focus |
|---|---|
| **v1.5 — Schedule + Bookings (PT slots) (next)** | `trainer_availability_slots` + `bookings` tables; race-safe partial-UNIQUE on confirmed booking per slot; FSM (`confirmed → cancelled / no_show / completed`); reception + owner + Telegram bot `/book` paths; PT-package linkage with decrement-at-delivery preserved; 5 new LOCKED audit events |
| **v1.6 — Email channel + Multi-user admin** | Email integration (Resend / SES / Mailgun — TBD) as a second notification channel; templates for OTP / expiring / payment-receipt fallback; owner-managed `POST /api/v1/users` so reception accounts can be onboarded without DB poking; soft-delete + deactivate; multi-user audit |
| **v1.7 — Online Payments (ЮKassa) + 54-ФЗ fiscal receipts** | ЮKassa payment intake (memberships + PT-packages) + webhook state machine + online refunds; ЮKassa `/receipts` integration (АТОЛ-Онлайн / Чек.ОФД path) for 54-ФЗ-compliant fiscal receipts on email/SMS to client. **Both bundled** — online sales cannot legally launch without fiscal receipts in РФ |
| **v1.8 — Reports + Audit Log read API** | Owner dashboard backend: revenue by day/month, active/expiring clients, top trainers, PT-session usage; `GET /api/v1/audit-log` with owner-only RBAC + filters (actor / resource / time-window / event-kind) |
| **v1.9 — API Handoff + Production Hardening** | (was v1.5) Curated Postman v2.1 collection + Newman CLI runner; auth runbook expansion; OpenAPI doc site + versioned spec URL; `@sportzal/api-client` publish to npm; OpenAPI tag curation + explicit `operation_id=`; idempotency hardening (CR-01/02/02b carry-over from Phase 33); residual DEFER-36-04-A (11 failures) + DEFER-36-04-B (ruff format 123 files); doc-debt sweep |
...
## Current Milestone: v1.5 Schedule + Bookings (PT slots)

**Goal:** Close the last empty business-module gap — give the gym a minimal viable online PT-booking surface so trainers can publish availability, clients can book individual sessions (reception, owner, or Telegram bot), and the booking lifecycle is race-safe + audit-traceable end-to-end. Group classes remain out of scope.

**Target features:**
- Trainer availability windows — `trainer_availability_slots` table (one-off and/or recurring slots; capacity = 1 for PT-individual)
- Race-safe booking — `bookings` table with partial UNIQUE `(slot_id) WHERE status='confirmed'` so concurrent same-slot bookings lose at the DB layer (mirrors v1.2 visits and v1.3 freeze-period discipline)
- Booking FSM — `confirmed → cancelled / no_show / completed`; reception ≤24h cancel, owner anytime (mirror of v1.4 B-12); `completed` set when a PT-session is recorded with the matching `booking_id`
- PT-package linkage — booking requires active PT-package with `sessions_remaining > 0`; resolver mirrors `register_active_pt_package_resolver`; **session decrement stays atomic on PT-session creation, not on booking** (D-37-XX to be locked in Phase 37)
- Telegram bot `/book` — client books a slot via bot when linked + has active PT-package
- 5 new locked audit events extending the v1.4 51-entry frozenset to 56

**Key context:**
- 4 phases estimated (37 → ~40). v1.5 is intentionally tight — supporting infra (email channel, multi-user admin) was split into v1.6 to avoid a dense 5–7-phase mega-milestone
- Modular monolith invariants must hold: new `app/modules/schedule/` + `app/modules/bookings/` are wired through Protocol slots from the `app/main.py` composition root; `import-linter` `modules-independent` contract gains 2 entries
- Inclusive date semantics + Europe/Moscow TZ + mandatory snapshot pricing patterns from v1.2–v1.4 apply where relevant
- This milestone introduces **no online-payment surface** — booking-without-payment is reception/owner-mediated; client self-book through bot still requires a pre-purchased PT-package (purchased via cash flow + receptionist for now; v1.7 will add the online path)
