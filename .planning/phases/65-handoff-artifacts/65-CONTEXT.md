...
2. **HND-02** — Pre-request/auth scripts: login extracts `Set-Cookie` (`sz_access`, `sz_refresh`, `sportzal_csrf` — the REAL cookie names set by `apps/backend/app/core/security.py`; the source-doc `cc_*` names are wrong — the rename is deferred to v2.0 per D-11-CSRF-DEFER) into collection variables; mutating requests auto-send `X-CSRF-Token` from the stored value; CSRF retrieval documented inline in the collection description.
