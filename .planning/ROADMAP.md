...
<details>
<summary>✅ v4.0 Production Infrastructure — Self-Hosted k3s (Phases 118-121) — SHIPPED 2026-06-16</summary>

Containerized the full stack + IaC (Terraform on-prem k3s) + observability (kube-prometheus-stack/Loki/Alloy) + backups (CNPG barman/Redis/SeaweedFS + restore-verify) + network security (Traefik v3 ingress, cert-manager, sealed-secrets, NetworkPolicies, hardened securityContext) + a Makefile CD layer with an 8-check smoke. **Done-bar = local validation** (k3d / `helm lint` / `terraform validate` / `make -n`); live-server apply + runtime legs are **operator-pending by design** (D-V40-LOCAL-VALIDATE). 39/39 requirements satisfied at that bar; audit passed; cross-phase wiring sound. Full phase details in [milestones/v4.0-ROADMAP.md](milestones/v4.0-ROADMAP.md). Operator-pending boundary — 23 items + 2 HARD gates (SEC-02 RSA-key off-node backup, BAK-03 verified restore round-trip) — in [../infra/runbooks/production.md](../infra/runbooks/production.md) and the per-phase `*-UAT.md` files.

- [x] **Phase 118: Container Images + Helm Chart (Core Stack)** (4 plans, IMG/DATA/APP) — completed 2026-06-16
- [x] **Phase 119: Networking, Security + CSRF Rename** (3 plans, NET/SEC) — completed 2026-06-16
- [x] **Phase 120: IaC, Observability + Backup** (3 plans, IAC/OBS/BAK) — completed 2026-06-16
- [x] **Phase 121: Makefile CI/CD + Full Smoke + Runbooks** (2 plans, OPS) — completed 2026-06-16
