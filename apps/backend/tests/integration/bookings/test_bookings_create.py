...
async def test_create_booking_pt_package_expired_before_slot_moscow_tz(
    ...
    01:00 Moscow on a far-future business date is 22:00 UTC the prior
    calendar day. A package with end_date == that prior day (Moscow
    business date) MUST reject the slot — naive .date() on the UTC
    datetime would mis-classify the slot as belonging to the prior day
    and incorrectly accept it. Dates are computed relative to
    datetime.now(UTC) (not hardcoded) so this test never expires as
    wall-clock time advances (V41-HYG-080).
    """
    ...
    # Slot at 01:00 Moscow on a business date far in the future → UTC is
    # 22:00 the prior calendar day.
    future_moscow_date = (datetime.now(UTC) + timedelta(days=365)).date()
    slot_start_moscow = datetime(
        future_moscow_date.year,
        future_moscow_date.month,
        future_moscow_date.day,
        1,
        0,
        tzinfo=moscow_tz,
    )
    ...
    pkg = await make_pt_package(
        client_id=client.id,
        plan=plan,
        # Moscow business date — strictly before slot.
        end_date=future_moscow_date - timedelta(days=1),
    )
