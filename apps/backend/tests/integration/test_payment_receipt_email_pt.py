"""Phase 45 NOTIFY-11/12/13 (Plan 45-10) — PT-package payment-receipt email
fanout integration tests.

Mirrors ``tests/integration/test_payment_receipt_email.py`` (Plan 45-09
memberships) byte-for-byte with PT-package routes / fixtures substituted.
Asserts D-45-08 best-effort post-commit fanout at the pt_packages
orchestrator sites: SALE (POST /api/v1/pt-packages) + REFUND (POST
/api/v1/pt-packages/{id}/refund) + skip-no-email (D-45-10).

Three cases:
  1. test_pt_package_sale_with_email_fanouts_receipt — client.email IS NOT
  ...
  2. test_pt_package_refund_with_email_fanouts_receipt — refund flow on the
     refund Payment row: receipt_kind="refund", template_id="EMAIL_PAYMENT_
     RECEIPT_REFUND".
  3. test_pt_package_sale_no_email_skips_fanout — client.email IS NULL:
     zero receipts + zero audits + zero recorder calls; PT-package sale
     succeeds (best-effort, D-45-08).

Runs on the SAVEPOINT-mode db_session — the orchestrator's first commit +
the helper's post-commit second commit BOTH become nested SAVEPOINT
releases under the outer rollback at teardown.
"""
...
from app.modules.payments.constants import SUBJECT_KIND_PT_PACKAGE, SUBJECT_KIND_REFUND
...
_OWNER_EMAIL = "receipt-flow-pt-owner@example.com"
...
# ---------------------------------------------------------------------------
# Local fixture machinery (mirrors tests/integration/test_payment_receipt_email.py).
# ---------------------------------------------------------------------------
...
async def _seeded_owner(
    ...
    user = User(
        ...
        role=Role.OWNER,
        full_name="Receipt Flow PT Owner",
    )
    db_session.add(user)
...
async def sandbox_email_recorder() -> AsyncIterator[_RecordingEmailDispatcher]:
    ...
    Mirrors tests/integration/test_payment_receipt_email.py fixture verbatim.
    """
...
def _sale_body(client_id: UUID, plan: PtPackagePlan) -> dict[str, Any]:
    return {
...
async def test_pt_package_sale_with_email_fanouts_receipt(
    ...
    plan = await _seed_pt_plan(db_session, name=f"SalePlan-{uuid4().hex[:6]}")
    client_row = await _seed_client(
        db_session,
        owner=_seeded_owner,
        phone="+79991230021",
        email="pt-sale-receipt@example.com",
    )

    r = await authed_owner_client.post(
        "/api/v1/pt-packages",
        json=_sale_body(client_row.id, plan),
        headers=_csrf_sale_headers(authed_owner_client),
    )
    assert r.status_code == 201, r.text
    pt_package_id = UUID(r.json()["data"]["id"])
    ...
    assert payload["to_email"] == "pt-sale-receipt@example.com"
    ...
    call = sale_calls[0]
    assert call["to"] == "pt-sale-receipt@example.com"
    ...
    # plan_snapshot reflects the PT-package plan name at sale time.
    assert call["plan_snapshot"] == plan.name
...
async def test_pt_package_refund_with_email_fanouts_receipt(
    authed_owner_client: AsyncClient,
    db_session: AsyncSession,
    sandbox_email_recorder: _RecordingEmailDispatcher,
    _seeded_owner: User,
    ...
    plan = await _seed_pt_plan(db_session, name=f"RefundPlan-{uuid4().hex[:6]}")
    client_row = await _seed_client(
        db_session,
        owner=_seeded_owner,
        phone="+79991230022",
        email="pt-refund-receipt@example.com",
    )
    ...
    call = refund_calls[0]
    assert call["to"] == "pt-refund-receipt@example.com"
...
async def test_pt_package_sale_no_email_skips_fanout(
    authed_owner_client: AsyncClient,
    db_session: AsyncSession,
    sandbox_email_recorder: _RecordingEmailDispatcher,
    _seeded_owner: User,
    ...
    plan = await _seed_pt_plan(db_session, name=f"NoEmailPlan-{uuid4().hex[:6]}")
    client_row = await _seed_client(
        db_session,
        owner=_seeded_owner,
        phone="+79991230023",
    ...
    # PT-package sale succeeds even without email (best-effort fanout).
    assert r.status_code == 201, r.text
    pt_package_id = UUID(r.json()["data"]["id"])
