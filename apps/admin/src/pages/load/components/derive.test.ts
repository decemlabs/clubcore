...
describe('derivePeakHour', () => {
  ...
  it('несортированные/разреженные бакеты с ничьёй → побеждает меньший hour (не порядок массива)', () => {
    // Sparse + unordered input: hour 18 appears BEFORE hour 9 in array order,
    // both tied at 15. Tie-break must pick hour 9 by VALUE, not array index.
    const sparse: VisitsReportHourlyBucket[] = [
      { hour: 18, count: 15 },
      { hour: 7, count: 3 },
      { hour: 9, count: 15 },
    ];
    const result = derivePeakHour(sparse);
    expect(result).not.toBeNull();
    expect(result!.hour).toBe(9);
    expect(result!.count).toBe(15);
  });
