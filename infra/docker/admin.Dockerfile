...
#
# admin production image: pnpm workspace build → nginx static serving.
...
#
# Build: docker build -f infra/docker/admin.Dockerfile -t clubcore/admin:<tag> .
...
# Copy only the packages that @clubcore/admin depends on.
...
# --filter admin... includes transitive workspace deps (api-client, ui).
RUN pnpm install --frozen-lockfile --filter @clubcore/admin...
...
# Build the admin (tsc -b && vite build → dist/).
RUN pnpm --filter @clubcore/admin build
...
COPY infra/nginx/admin.conf /etc/nginx/nginx.conf

# Copy built static assets from build stage.
COPY --from=builder --chown=nginx:nginx /workspace/apps/admin/dist /usr/share/nginx/html
